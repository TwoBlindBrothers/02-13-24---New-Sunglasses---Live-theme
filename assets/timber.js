...
timber.cacheSelectors = function () {
  timber.cache = {
    ...
    $featuredImagesOwl       : $('.product-images-carousel'),
...
timber.productPage = function (options) {
  ...
  var $productImage = $('#ProductPhotoImg'),
      $variantTitle = $('#variantTitle,#variantTitle_m'),
      $variantMeta = $(".variant-meta");
      $imagesMainCarousel = $('.product-images-carousel'),
      $addToCart = $('#AddToCart'),
      $productPrice = $('#ProductPrice,#ProductPrice_d'),
      $comparePrice = $('#ComparePrice'),
      $quantityElements = $('.quantity-selector, label + .js-qty'),
      $addToCartText = $('#AddToCartText');

  
  // if ( $imagesMainCarousel.length ) {
  //   $imagesMainCarousel.on('to.owl.carousel, next.owl.carousel, prev.owl.carousel, destroy.owl.carousel', function(e) { console.log('hellllloooo'); e.stopPropagation(); });
  // }
  
  if (variant) {
      //Update product title to selected variant

    var name = $(".variant-meta-data[data-variant=" + variant.id + "]").html();
    if ( variant.option1 !== 'Default Title' ) {
      if ( $variantTitle.next("span").data('toggle') !== false ) {
        $variantTitle.next("span").html(variant.option1);
      }
    }

    // Update variant image, if one is set
    if (variant.featured_image) {
      if ( $productImage.hasClass('has_alt') ) {
        return
      }
    ...
    const cartCurrency = window.Shopify.currency.active === 'CAD' ? ' CAD' : '';
    $productPrice.html( Shopify.formatMoney(variant.price, moneyFormat) + cartCurrency);
  ...
  if (timber.cache.$thumbImages.length) {
    ...
    timber.cache.$thumbImages.on('click', function(evt) {
      ...
      if ( $imagesMainCarousel.length ) {
        ...
        $imagesMainCarousel.trigger('to.owl.carousel', imageIndex)
      }
      // var newImage = $(this).attr('href');
      // var newImageZoom = $(this).data('zoom');
...
if ( $('.hamburger').length ) {
  ...
  var focusableEls = $('#menu-overlay a[href], #sticky-header a[href]');
  // overlayMenu.querySelectorAll('a[href]');
  // var headerFocusableEls = document.getElementById('sticky-header').querySelectorAll('a[href]');

  // headerFocusableEls = Array.prototype.slice.call(headerFocusableEls);
  focusableEls = Array.prototype.slice.call(focusableEls);
  // focusableEls = focusableEls + headerFocusableEls;
  $(document).on('keydown', function (e) {
    if ( e.keyCode === 27 && $('.hamburger').hasClass('is-active') ) {
      $('.hamburger').removeClass('is-active');
      $('body').removeClass('menu-active')
      $('#menu-overlay-content').attr('aria-hidden', true)
      $('#menu-overlay-content').removeAttr("tabindex");
      $('.main-content, .site-header, .site-footer').attr('aria-hidden', 'false')
      $('.hamburger').focus()
    }
  });
  let hasVisit = false;
  $('.hamburger').on('keydown', function(e) {
    if ( e.keyCode === 9 && $(e.target).hasClass('is-active') ) {
      if (!hasVisit) {
        e.preventDefault();
  ...
  // $('.site-search-link').on('keydown', function(e) {
  //   console.log('here');
  //   if (e.keyCode === 9) {
  //     if($('.site-search-link').attr('aria-expanded') == 'true') {
  //       e.preventDefault();
  //       $('form#searchForm input[type="search"]').focus();
  //     }
  //   }
  // });
  $('#menu-overlay-content a, #sticky-header a').on('keydown', function (e) {
    if ( e.keyCode === 9 && $('.hamburger').hasClass('is-active') ) {
      if ( e.shiftKey ) {
  ...
  $('.hamburger').on('click', function(e) {
    lastFocus = this || document.activeElement;
    if ( $('.hamburger').hasClass('is-active') ) {
      ...
      podcastPixel.id = 'podcast-pixel-hamburger'
      podcastPixel.src="https://data.adxcel-ec2.com/pixel/?ad_log=referer&action=misc&pixid=e503aefe-73f2-46f2-8010-e12740208050";
      $(this).attr('aria-expanded', true);
      $(this).attr('aria-label', 'Main menu expanded');
      $('.hamburger').addClass('is-active');
      $('body').addClass('menu-active');
      $('#menu-overlay').attr('aria-hidden', false)
      $('#menu-overlay').focus()
      $('#menu-overlay-content').attr("tabindex", "-1");
      const headerHeight = $('.header-wrapper').height()
      $('#menu-overlay').css({ height: window.innerHeight - headerHeight - 1 + 'px', top: headerHeight - 1 + 'px' })
      $('.main-content, .site-header, .site-footer').attr('aria-hidden', 'true')
    }
  })
  var container = $("#menu-overlay-content");
  $('#menu-overlay').on('click', function(e) {
    if ( !$(e.target).closest(container).length && $('.hamburger').hasClass('is-active') ) {
      $('.hamburger').removeClass('is-active');
      $('body').removeClass('menu-active')
      $('#menu-overlay').attr('aria-hidden', true)
      $('#menu-overlay-content').removeAttr("tabindex");
...
timber.switchImage = function (src, imgObject, el) {
    ...
    var $el = $(el);
    // $el.attr('src', src);
    // $el.data('zoom', zoomImg);
    // if ( $('.zoom-image').length ) {
    //     if ( $el.parent().find('.zoomImg') ) {
...
timber.responsiveVideos = function () {
  ...
  $iframeVideo.each(function () {
    ...
    $(this).wrap('<div class="video-wrapper-container"><div class="video-wrapper"></div></div>');
  });
...
timber.Drawers = (function () {
  var Drawer = function (id, position, options) {
    ...
    this.$nodes = {
      ...
      background: $('#cartDrawerBackground')
    };
  ...
  Drawer.prototype.open = function (evt) {
    ...
    // Show Rebuy smart cart if available
    if (window.Rebuy?.SmartCart) return window.Rebuy.SmartCart.show();
    ...
    this.$nodes.background.on('click.drawer', $.proxy(function () {
      this.close();
      return false;
    }, this));
...
$( ".site-search-link" ).click(function(e) {
  if($(".site-search-link").attr('aria-expanded') == 'true') {
    $(".site-search-link").attr('aria-expanded', 'false');
  } else {
    $(".site-search-link").attr('aria-expanded', 'true');
  }
  e.preventDefault();
  $( '.site-header input' ).toggle( "fast", function() {
    $(this).width( 300 ).addClass( "search-open" );
  });
});
